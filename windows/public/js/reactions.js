...
const Reactions = (() => {
  ...
  function warningFor(m) {
    if (m.type === 'image') {
      ...
      if (_catalog.images.length && !_catalog.images.includes(m.value)) return `“${m.value}” is missing from reactions/assets/.`;
      return '';
    }
  ...
  function renderNote() {
    ...
    note.innerHTML = `
      Drop your own PNG / GIF memes into <code>windows/reactions/assets/</code> (or use Upload) and pick them as <b>Image</b>.
      Bake extra emoji with <code>python tools/render_emoji_assets.py 🦄</code>.<br>${renderLine}<br>
      New gestures live in <code>bridge_py/reactions/triggers.py</code>, new motion presets in <code>animations.py</code>.`;
  }
