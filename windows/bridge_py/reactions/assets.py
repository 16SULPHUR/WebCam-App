...
Resolves a mapping to an RGBA Sprite:
  type "emoji" → reactions/assets/emoji/<codepoints>.png, rendered on demand
                 from the system emoji font (Pillow) when not bundled
  type "image" → any PNG / JPG / WEBP / GIF inside reactions/assets/
"""
...
from .common import (EMOJI_DIRNAME, bundled_emoji, emoji_filename,
                     emoji_font_path, list_images)
...
class AssetLibrary:
    def __init__(self, assets_dir: str):
        self.dir = assets_dir
        self.emoji_dir = os.path.join(assets_dir, EMOJI_DIRNAME)
    ...
    def _safe_path(self, rel: str) -> str | None:
        candidate = os.path.realpath(os.path.join(self.dir, rel))
        if not candidate.startswith(os.path.realpath(self.dir)):
    ...
    def _emoji_sprite(self, char: str) -> Sprite | None:
        path = os.path.join(self.emoji_dir, emoji_filename(char))
