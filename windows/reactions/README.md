...
windows/
├── reactions/
│   ├── assets/            ← your memes (png / jpg / webp / gif)
│   │   └── emoji/         ← baked emoji sprites (generated)
│   └── README.md
├── tools/render_emoji_assets.py   ← bakes emoji → png
└── bridge_py/reactions/
...
**A meme image** — drop a PNG/JPG/WEBP/GIF into `reactions/assets/` (or use
...
first time it fires and cached in `assets/emoji/`.

To pre-bake extras so they work without Pillow:

```bash
cd windows
python tools/render_emoji_assets.py 🦄 🫠 🥹
```

## Adding a gesture

Write a rule in `bridge_py/reactions/triggers.py` — it appears in the settings
...
```bash
pip install mediapipe        # 0.10.x recommended
pip install pillow           # optional: custom emoji + animated GIF support
```

On MediaPipe **1.0+** the legacy `mp.solutions` graphs are gone, so the detector
falls back to the Tasks API and needs the model files in `windows/tools/models/`
(or set `MEDIAPIPE_MODELS_DIR`, e.g. `F:\tools\mediapipe\models`):
